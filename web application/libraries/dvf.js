...
function dvf2(json,value1,value2,currentmap,mapnamestring,check_choice,value3){
...
// Check max
	for (var i=0 ; i< json.features.length ; i++) {
		if (i==0){
			var max =parseFloat(json.features[0][value1][value2]);
		}
		else{
			if (parseFloat(json.features[i][value1][value2]) > max){
				max =parseFloat(json.features[i][value1][value2]);
	...
	for (var y=0 ; y< json.features.length ; y++) {
		if (y==0){
			var min =parseFloat(json.features[0][value1][value2]);
		}
		else{
			if (parseFloat(json.features[y][value1][value2]) < min){
				min =parseFloat(json.features[y][value1][value2]);
...
//Setup mapping between number of electoral votes and color/fillColor.   In this case, we're going to vary color from green (hue of 120) to red (hue of 0) with a darker border (lightness of 25%) and lighter fill (lightness of 50%)
	...
	dvf_display(json,value1,value2,max,min,check_choice,value3);
...
function dvf4(json,value1,value2,currentmap,mapnamestring,check_choice,value3)
...
function dvf3(json,value1,value2,currentmap,mapnamestring,check_choice,value3)
// json is the array
// value1 and value2 describes the path of the array
{
// Display the name of the municipality on hover- controllled by value3
var value3="Name";
